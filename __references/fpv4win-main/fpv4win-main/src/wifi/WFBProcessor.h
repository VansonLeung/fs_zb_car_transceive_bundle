...
public:
    ...
    virtual void
    process_packet(const uint8_t *buf, size_t size, uint8_t wlan_idx, const uint8_t *antenna, const int8_t *rssi);
    uint32_t GetDecryptErrorCount() const { return count_p_dec_err; }
    uint32_t GetDecodedPacketCount() const { return count_p_dec_ok; }
    uint32_t GetBadPacketCount() const { return count_p_bad; }
    bool HasActiveSession() const { return fec_p != nullptr; }
